...
/**
 * Whether a settled transcript block paints anything. Only a content-bearing
 * tool/reasoning trail whose every section is hidden (e.g. `/details hidden`)
 * renders nothing — every other block draws at least one row. A block that
 ...
 * a hidden trail never leaves a floating blank line or doubles the gap after a
 * user prompt. In the default/collapsed modes trails always render, so this is
 * a no-op there.
 */
